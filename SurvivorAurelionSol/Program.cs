// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Program.cs" company="SurvivorAurelionSol">
//      Copyright (c) SurvivorAurelionSol. All rights reserved.
// </copyright>
// --------------------------------------------------------------------------------------------------------------------
...
namespace SurvivorAurelionSol
{
    internal class SurvivorAurelionSol
    {
        ...
        {
            var SurvivorAurelionSol = new SurvivorAurelionSol();
        }
        ...
        private void GameOnGameLoad(EventArgs args)
        {
            ...
            Q = new Spell(SpellSlot.Q, 2000f);
            W = new Spell(SpellSlot.W, 600f);
            E = new Spell(SpellSlot.E, 3000f);
            R = new Spell(SpellSlot.R, 1300);
            Q.SetSkillshot(0.5f, 65f, 1300f, true, SkillshotType.SkillshotLine);
                // OnQ Started increase Width every tick by 1 or so
            R.SetSkillshot(0.5f, 100f, 1200f, false, SkillshotType.SkillshotLine);
            ...
            Config = new Menu("SurvivorAurelionSol", "SurvivorAurelionSol", true).SetFontStyle(FontStyle.Bold,
                Color.Chartreuse);
            ...
            var SkinID =
                SkinChangerMenu.AddItem(
                    new MenuItem("SkinID", ":: Skin").SetValue(
                            new StringList(new[] {"Classic", "Fire Aurelion Sol"}, 0))
                        .SetFontStyle(FontStyle.Bold, Color.Crimson));
                ...
                if (eventArgs.GetNewValue<StringList>().SelectedIndex == 2)
                    if (!SPredictionLoaded)
                    {
                        ...
                        var SPreditctionLoaded =
                            MiscMenu.AddItem(new MenuItem("SPredictionLoaded", "SPrediction Loaded!"));
                        Game.PrintChat(
                            "<font color='#0993F9'>[SS Aurelion Sol Warning]</font> <font color='#FF8800'>Please exit the menu and click back on it again, to see the settings or Reload (F5)</font>");
            ...
            var drawdamage = DrawingMenu.AddSubMenu(new Menu(":: Draw Damage", "drawdamage"));
            {
                var dmgAfterShave =
                    drawdamage.AddItem(
                        new MenuItem("SurvivorAurelionSol.DrawComboDamage", "Draw Damage on Enemy's HP Bar").SetValue(
                            true));
                var drawFill =
                    drawdamage.AddItem(new MenuItem("SurvivorAurelionSol.DrawColour", "Fill Color", true).SetValue(
            ...
            Game.PrintChat(
                "<font color='#800040'>[SurvivorSeries] AurelionSol</font> <font color='#ff6600'>Loaded.</font>");
        ...
        {
            /*if (sender.IsValid && sender.IsAlly && sender is Obj_AI_Minion &&
                (sender.Name.ToLower() == "AurelionSolminion"))
                Daisy = null;*/
        }
        ...
        private void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            /*if (Q.Instance.IsReady())
            {
                ...
                if (Config.Item("GapCloserEnemies" + GapCloser.ChampionName).GetValue<bool>() &&
                    GapCloser.IsValidTarget(400) && !Player.HasBuff("AurelionSolqallyjump"))
        ...
        {
            if (Config.Item("QToInterrupt").GetValue<bool>() && !Player.HasBuff("AurelionSolqallyjump") &&
        ...
        private void Obj_AI_Base_OnCreate(GameObject obj, EventArgs args)
        {
            if (Player.Distance(obj.Position) < 300)
                Game.PrintChat(obj.Name + " | " + obj.Flags + " | " + obj.Type + " | " + obj.Position);
            /*if (obj.Name == "AurelionSol_Base_P_cas.troy")
                Notifications.AddNotification(
                    new Notification("Jungle Camp claiming.", 4000).SetTextColor(
                        System.Drawing.Color.Chartreuse));
            if (obj.Name == "AurelionSol_Base_P_FinishedGround.troy")
                Notifications.AddNotification(
                    new Notification("Jungle Camp is now ready.", 4000).SetTextColor(
                        System.Drawing.Color.Chartreuse));
            if (obj.IsValid && obj.IsAlly && obj is Obj_AI_Minion && (obj.Name.ToLower() == "AurelionSolminion"))
        ...
        private void Combo()
        {
            ...
            if ((target == null) || !target.IsValidTarget())
                return;
            if (Player.HasBuff("AurelionSolqallyjump") && Config.Item("ComboDashIfPossibleQ").GetValue<bool>() &&
                (target.CountEnemiesInRange(1500) <= 1))
                Q.Cast();
            if (UseE && E.Instance.IsReady() && target.IsValidTarget(E.Range))
                E.CastOnUnit(Player);
            if (UseQ && Q.Instance.IsReady() && !Player.HasBuff("AurelionSolqallyjump"))
                SebbySpell(Q, target);
            ...
            if (UseW && W.Instance.IsReady() && (target.Distance(Player) > Player.AttackRange) &&
                (target.Distance(Player) < 450) && !Player.HasBuff("AurelionSolW"))
        ...
        private void TakingFatalDamageCheck()
        {
            ...
            if (Config.Item("UseENearbyEnemy").GetValue<bool>() &&
                (Player.CountEnemiesInRange(Config.Item("MinimumEnemiesNearEDistance").GetValue<Slider>().Value) >=
        ...
        private void HarassMode()
        {
            ...
            if (UseW && (target.Distance(Player) > Player.AttackRange) && (target.Distance(Player) < 450) &&
                !Player.HasBuff("AurelionSolW") && W.Instance.IsReady())
                W.Cast(Player.ServerPosition);
        ...
        private void DaisyProgram()
        {
            if (R.Instance.IsReady())
                if (Config.Item("AutoDaisy").GetValue<bool>() && (Daisy != null) && Daisy.IsValid)
                {
                    ...
                    {
                        var AurelionSoltarget = Orbwalker.GetTarget() as Obj_AI_Base;
                        if (AurelionSoltarget != null)
                            if (Daisy.Distance(AurelionSoltarget.Position) > 200)
                            {
                                Player.IssueOrder(GameObjectOrder.MovePet, AurelionSoltarget);
                                DaisyStatus = true;
                            }
                            else
                            {
                                Daisy.IssueOrder(GameObjectOrder.AttackUnit, AurelionSoltarget);
                                DaisyStatus = true;
                                E.CastOnUnit(Daisy);
                            }
