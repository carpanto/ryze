    ...
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.IsEnemy))
            {
                ...
                MiscMenu.SubMenu("[Blacklist] W").AddItem(new MenuItem("BlackList" + enemy.ChampionName, enemy.ChampionName).SetValue(true).SetTooltip("Blacklist from using W at the disabled (false) champion"));
            }
        ...
        private static void KSAshe()
        {
            if (Menu.Item("KillSteal").GetValue<bool>())
            {
                ...
                if (Menu.Item("BlackList" + target.ChampionName).GetValue<bool>() && target.IsValidTarget(W.Range) && target.Health < W.GetDamage(target) + OktwCommon.GetIncomingDamage(target))
                {
                    if (target.CanMove)
                    {
                        SebbySpell(W, target);
        ...
        private static void Combo()
        {
            ...
            if (target != null && target.IsValidTarget())
            {
                if (Menu.Item("ComboUseQ").GetValue<bool>() && target.IsValidTarget(SebbyLib.Orbwalking.GetRealAutoAttackRange(null)) && Q.IsReady() && Player.Mana > Q.Instance.ManaCost + R.Instance.ManaCost || target.Health < 5 * Player.GetAutoAttackDamage(Player))
                    Q.Cast();
                if (Menu.Item("BlackList" + target.ChampionName).GetValue<bool>() && Menu.Item("ComboUseW").GetValue<bool>() && Menu.Item("BlackList" + target.ChampionName).GetValue<bool>() && W.IsReady() && Player.Mana > W.Instance.ManaCost + R.Instance.ManaCost && target.IsValidTarget(W.Range))
                    SebbySpell(W, target);
        ...
        private static void Harass()
        {
            ...
            if (target != null && target.IsValidTarget())
            {
                if (Menu.Item("BlackList" + target.ChampionName).GetValue<bool>() && target.IsValidTarget(W.Range) && SpellW)
                    SebbySpell(W, target);
